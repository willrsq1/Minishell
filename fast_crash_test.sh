...
do
  ...
  # Step 2
  cat /dev/urandom | valgrind --leak-check=full --show-leak-kinds=all --track-fds=yes ./minishell  2>>err.txt 1>&2 &
  ...
  grep -an "command not" err.txt | sed 's/Minishell d'\''Arbesa > //g'  >> commands.txt
  ...
  echo "Find the commands inputed in commands.txt."
